class LimitationsViewer {
	...
	initSliderSync() {
		...
		if (!this._boundTimeupdate) {
			this._boundTimeupdate = () => {
				...
				let lk = Math.min(Math.floor(progress * denom), denom - 1);
				let k  = this.isPresent() ? (this.presentMin() + lk) : lk;
	...
	loadVideos() {
		...
		if (this.method === 'pastfuture') {
			this.motionetr_recon.src = method_not_supported; this.motionetr_recon.load(); this.motionetr_recon.currentTime = 0;
			this.motionetr_tracks.src = method_not_supported; this.motionetr_tracks.load(); this.motionetr_tracks.currentTime = 0;
			this.jin_recon.src = method_not_supported; this.jin_recon.load(); this.jin_recon.currentTime = 0;
			this.jin_tracks.src = method_not_supported; this.jin_tracks.load(); this.jin_tracks.currentTime = 0;
		} else {
